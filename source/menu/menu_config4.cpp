...
void CMenu::_showConfig4(void)
{
	...
	m_btnMgr.setText(m_config4BtnHome, m_cfg.getInt("GENERAL", "exit_to_wii_menu") ? gOn : gOff);
	m_btnMgr.setText(m_config4BtnSaveFavMode, m_cfg.getInt("GENERAL", "favorites_on_startup") ? gOn : gOff);
	m_btnMgr.setText(m_config4BtnSearchMode, m_cfg.getInt("GENERAL", "alphabetic_search_on_plus_minus") ? _t("smalpha", "Alphabetic") : _t("smpage", "Pages"));
}
...
//	u32 btn;
	...
	while (true)
	{
		...
		if ((padsState & WPAD_BUTTON_A) != 0)
		{
			...
			{
				m_cfg.setInt("GENERAL", "exit_to_wii_menu", !m_cfg.getInt("GENERAL", "exit_to_wii_menu"));
				Sys_ExitToWiiMenu(m_noHBC || m_cfg.getInt("GENERAL", "exit_to_wii_menu", false));
				_showConfig4();
			}
			else if (m_btnMgr.selected() == m_config4BtnSaveFavMode)
			{
				m_cfg.setInt("GENERAL", "favorites_on_startup", !m_cfg.getInt("GENERAL", "favorites_on_startup"));
				_showConfig4();
			}
			else if (m_btnMgr.selected() == m_config4BtnSearchMode)
			{
				m_alphaSearch = !m_cfg.getInt("GENERAL", "alphabetic_search_on_plus_minus", false);
				m_cfg.setInt("GENERAL", "alphabetic_search_on_plus_minus", m_alphaSearch);
				_showConfig4();
			}
		}
		_mainLoopCommon(wd);
