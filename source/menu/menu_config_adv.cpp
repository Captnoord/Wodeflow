...
void CMenu::_showConfigAdv(void)
{
	...
	m_btnMgr.setText(m_configAdvLblCurTheme, m_cfg.getStr("GENERAL", "theme"));
}
...
int CMenu::_configAdv(void)
{
	...
	int curTheme;
	string prevTheme = m_cfg.getStr("GENERAL", "theme");
	...
	while (true)
	{
		...
		if ((padsState & WPAD_BUTTON_A) != 0)
		{
			...
			else if (m_btnMgr.selected() == m_configAdvBtnCurThemeP)
			{
				...
				m_cfg.setStr("GENERAL", "theme", themes[curTheme]);
				_showConfigAdv();
			}
			...
			else if (m_btnMgr.selected() == m_configAdvBtnCurLanguageP)
			{
				...
				m_cfg.setStr("GENERAL", "language", m_curLanguage);
				_updateText();
				_showConfigAdv();
			}
