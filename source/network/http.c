...
int findHeader(char *headers, int length, char *header, char *buf, int bufLength)
{
	char *completeHeader = strstr(headers, header);
	if ((completeHeader - headers) > length || completeHeader == NULL) {
		return -1;
	}
	
	int hdrLen = strstr(completeHeader, "\r\n") - completeHeader;
	
	int hLen = strlen(header) + 2;
	if (hdrLen - hLen > bufLength) {
		return -1;
	}
	
	memset(buf, 0, bufLength);
	strncpy(buf, completeHeader + hLen, hdrLen - hLen);
	return 0;
}
